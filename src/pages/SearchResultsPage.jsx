...
const SearchResultsPage = () => {
 ...
 const [loading, setLoading] = useState(false);
  ...
  const handleSearch = async (e) => {
   e.preventDefault();
   setLoading(true);
   console.log("Searching for flats...");
    ...
    <div className="flex min-h-screen bg-gray-100">
      ...
      <aside className="w-64 p-6 bg-slate-50 shadow-md">
        ...
        <div className="flex gap-2 mt-6">
          <button
            className="bg-[#8E05C2] text-white px-4 py-2 rounded "
            ...
            {flats.map((flat) => (
              <div key={flat.id} className="bg-white shadow-md rounded-lg overflow-hidden">
                ...
                <div className="p-4">
                  ...
                  <p className="text-[#8E05C2] font-semibold">₹{flat.price}/month</p>
                  <p className="text-sm text-gray-500 mt-2">Type: {flat.type}</p>
                  <p className="text-sm text-gray-500">Amenities: {flat.type}</p>
                  <button className="mt-3 inline-block bg-[#8E05C2] text-white px-4 py-2 rounded hover:bg-blue-700">
                    View Details
                  </button>
                </div>
              </div>
